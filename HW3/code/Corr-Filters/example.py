...
def animate(i):
    ...
    if i < N:  # If the animation is still running
        ...
        plt.show()
        lamb = 1
        g = np.matmul(np.matmul(np.linalg.inv(np.matmul(X.T, X) + lamb * np.identity(X.shape[0])), X.T), Y)
        g = np.reshape(g, (patch.get_height(), patch.get_width()))
        filtered_lena = correlate(img, g)
        plt.figure()
        ...
        plt.imshow(g)
        plt.show()
        conv_lena = convolve(img, g)
        plt.figure()
        plt.imshow(conv_lena)
        plt.show()
