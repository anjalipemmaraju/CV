...
def LucasKanadeBasis(It, It1, rect, bases):
	...
	# create bivariate spline to do pixel interpolation
	...
	Z = np.identity(B.shape[0]) - np.matmul(B,B.T)
	while norm >= threshold:
		...
		# find the difference between the bases and your warped frame / warped gradients
		delta_p, _, _, _ = np.linalg.lstsq(np.matmul(Z,A), np.matmul(Z,b), rcond=None)
